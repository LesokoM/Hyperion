...
def find_usernames_and_passwords():
    ...
    full_path = os.path.join(abso_path, relative_path)
    user_login_details = open(full_path, "r")
    usernames = []
    passwords = []
...
def login(database_usernames, database_passwords):
    ...
    while True:
        ...
        if input_username in database_usernames:
            ...
            index = database_usernames.index(
                input_username
            )  # idea comes from https://www.geeksforgeeks.org/python-list-index/
            while (
                number_of_tries > 0
            ):  # ensures thetre is a limit to the number of password tries
                input_password = input("Please enter password: ")
                if (
                    input_password == database_passwords[index]
                ):  # if username/password pair is correct quit the login loop
                    print("Correct username and password")
                    menu(input_username)
                else:
                    number_of_tries -= 1
                    print(
                        f"Incorrect password. Number of tries left: {number_of_tries}"
                    )
        else:
...
def validate_date():
    ...
    while True:
        date = input(
            "Please enter due date in the following format dd/mm/yyyy: "
        )  # getting user date
        try:
            date = datetime.strptime(
                date, "%d/%m/%Y"
            )  # converting entry into day, month and year
            if date < current_date:
                print(
                    "Invalid date entered. Due date needs to be ahead of time"
                )  # making sure due date is ahead not behind current date
            else:
                new_date = date.strftime(
                    "%d %B %Y"
                )  # changing month to the month name not value
                return new_date
        except ValueError:
            print(
                "Invalid date entered please try again"
            )  # if numbers that are not correct date or incorrect format is entered except will deal with it

def admin_stats():
    abso_path_u = os.path.dirname(__file__)
    relative_path_u = "user.txt"
    full_path_user = os.path.join(abso_path_u, relative_path_u)
    abso_path_t = os.path.dirname(__file__)
    relative_path_t = "tasks.txt"
    full_path_tasks = os.path.join(abso_path_t, relative_path_t)
    with open(full_path_user, "r") as task_stat:
        lines = len(task_stat.readlines())
    with open(full_path_tasks, "r") as user_stat:
        ...
        if menu == "s":
            ...
            if active_user == "admin":
                ...
                while True:
                    ...
                    if new_username not in database_usernames:
                        ...
                        print(
                            f"Your new username is {new_username} and new password is {new_password}"
                        )  # displaying new logins to users
                        database_usernames.append(new_username)
                        database_passwords.append(new_password)
                        break
                    else:
                        print(
                            "That username is already taken. Please enter a different one."
                        )
                        # if username already made user prompted to enter a different one
                # write_to_file = open("LEVEL1\\PART_17\\user.txt", "a")
                abso_path = os.path.dirname(__file__)  # directory user is working
                relative_path = "user.txt"
                full_path = os.path.join(abso_path, relative_path)  # fuction
                write_to_file = open(full_path, "a")
            ...
            while True:
                try:
                    ...
                    task_to_which_user = int(
                        input("Please select which user to assign the task to: ")
                    )
                    # ensuring they choose a value that is actually listed
                    if (
                        task_to_which_user > len(database_usernames)
                        or task_to_which_user <= 0
                    ):
                        print(
                            "You have selected a number not on the list. Please try again"
                        )
                    else:
            ...
            abso_path = os.path.dirname(__file__)
            relative_path = "tasks.txt"
            full_path = os.path.join(abso_path, relative_path)
            write_to_taskfile = open(full_path, "a+")
            # writing to file
            write_to_taskfile.write(
                f"\n{database_usernames[task_to_which_user]}, {task_title}, {task_description}, {now_date}, {due_date}, No"
            )
            # closing file
            ...
            relative_path = "tasks.txt"
            full_path = os.path.join(abso_path, relative_path)
            task_contents = open(full_path, "r")
            ...
            relative_path = "tasks.txt"
            full_path = os.path.join(abso_path, relative_path)
            vm_task_contents = open(full_path, "r")
